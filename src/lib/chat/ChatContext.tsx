...
interface ChatContextType {
  ...
  shareThread: (
    threadId: string,
    shareData: ThreadShare
  ) => Promise<ThreadShareResponse>; // Updated return type
...
export function ChatProvider({ children }: ChatProviderProps) {
  ...
  const [userRole, setUserRole] = useState<'owner' | 'contributor' | 'viewer'>(
    'viewer'
  );
  ...
  useEffect(() => {
    ...
    setCurrentThread((prev) => {
      ...
      // Preserve existing messages if wsMessages is empty
      const updatedMessages =
        wsMessages.length > 0 ? wsMessages : prev.messages || [];

      const updatedMessageCount =
        wsMessages.length > 0
          ? wsMessages.filter((m) => !m.isStreaming).length
          : prev.message_count || 0;

      const updatedLastMessageAt =
        wsMessages.length > 0
          ? wsMessages[wsMessages.length - 1]?.created_at ||
      ...
      return {
        ...
        last_message_at: updatedLastMessageAt,
      };
    });
  ...
  const updateThread = useCallback(
    async (threadId: string, updates: ThreadUpdate) => {
      if (!user?.id || !token) {
        throw new Error('Authentication required');
      }
  ...
  const shareThread = useCallback(
    async (threadId: string, shareData: ThreadShare): Promise<ThreadShareResponse> => {
      if (!user?.id || !token) {
        throw new Error('Authentication required');
      }
  ...
  const getThreadAccess = useCallback(
    async (threadId: string, shareId?: string) => {
      ...
      try {
        ...
        setUserRole(data.role); // Ensure userRole is updated
        setError(null);
      } catch (err) {
  ...
  const acceptThreadInvite = useCallback(
    async (threadId: string, shareId: string) => {
      if (!user?.id || !token) {
        throw new Error('Authentication required');
      }
  ...
  useEffect(() => {
    setLoading((prevLoading) => prevLoading || wsLoading);
  }, [wsLoading]);
